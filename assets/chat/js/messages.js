...
import {EmoteFormatter, GreenTextFormatter, HtmlTextFormatter, MentionedUserFormatter, UrlFormatter, IdentityFormatter, SpoilerFormatter} from './formatters'
...
// formatters.set('url', new IdentityFormatter())
...
function setFormattersFromSettings(settings) {
    ...
    // if (settings.get('formatter-antilinkgore') === true) formatters.set('url', new UrlFormatter({antiLinkGore: true}));
    // else if (settings.get('formatter-antilinkgore') === false) formatters.set('url', new UrlFormatter({antiLinkGore: false}));
}
...
function ChatEmoteMessageCount(message){
    ...
    message._text.attr('class', 'emote-combo' + stepClass)
    message._combo_count.text(`${message.emotecount}`)
    message._combo_user.text(`${message.user||""}`)
    //console.log(`${message.username||message.message}`)
    //if(message.user){console.log(`${message.user||message.message}`)}
    //if(message.user){console.log(`${message.user.username||message.message}`)}
    //if(message.message.user){console.log(`${message.message.user.username||message.message}`)}
...
class ChatEmoteMessage extends ChatMessage {
    ...
    // Push the combo further to the right, with this position, it loses width of emote
    html(chat=null){
        this._text          = $(`<span class="text" style="position:absolute;z-index:1;padding: 7px 0px 0px 12px;">${formatters.get('emote').format(chat, this.message, this)}</span>`)
        this._combo         = $(`<span class="chat-combo"></span>`)
        this._combo_count   = $(`<i class="count">${this.emotecount}</i>`)
        this._combo_x       = $(`<i class="x">x</i>`)
        this._combo_hits    = $(`<i class="hit">TIMES</i>`)
        this._combo_txt     = $(`<i class="combo">COMBO</i>`)
        this._combo_user    = $(`<span class="combouser">${this.combouser}</span>`)
