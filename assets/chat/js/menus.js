...
function buildEmote(emote){
    return `<div class="emote" style="padding: 0.1em;"><span title="${emote}" class="chat-emote chat-emote-${emote}">${emote}</span></div>`
}
    ...
    addElement(username, sort=false){
        ...
        const user = this.chat.users.get(username.toLowerCase()),
          ...
          features = user.features.length === 0 ? 'nofeature' : user.features.join(' '),
               usr = $(`<a data-username="${user.username}" class="user ${features} ${tag}">${label}</a>`)
...
class ChatEmoteMenu extends ChatMenu {
    ...
    constructor(ui, btn, chat) {
        ...
        //    <div style="display:block;margin-top:10px;margin-right:auto;margin-left:auto;"></br>
        //    <center><p><b>Sub Only Emotes</b></p></center></div>
        //    </br>
        //`)
        this.semotes = this.ui.find('#sub-emotes');
        this.demotes = this.ui.find('#default-emotes');
        this.demotes.append([...this.chat.emoticons].map(buildEmote).join(''));
        //this.demotes.append(this.subonlymessage);
        this.semotes.append([...this.chat.subemotes].map(buildEmote).join(''));
...
class ChatWhisperUsers extends ChatMenu {
    ...
    constructor(ui, btn, chat){
        super(ui, btn, chat);
        this.unread = 0;
        this.empty = $(`<span class="emptyw">No unread DMs</br></br><a target="_blank" href="/profile/messages">Check all your messages here</a></span>`);
    ...
    updateNotification(){
        ...
        try{ // Add the number of unread items to the window title.
            ...
            window.parent.document.title = this.unread > 0 ? `${this.chat.config.meta['title']} - Chat (${this.unread})` : `${this.chat.config.meta['title']} - Chat`;
    ...
    redraw(){
        ...
        if(this.visible){
            ...
            if(this.chat.whispers.size === 0) {
                ...
                .forEach(e => this.addConversationRead(e[0], e[1].read));*/
            }
        }
        super.redraw();
    }
   ...
   /* addConversationRead(nick, read){
        const user = this.chat.users.get(nick.toLowerCase()) || new ChatUser(nick)
        this.usersEl.append(`
            <li class="conversation unread-0">
                <a data-username="${user.nick.toLowerCase()}" title="Hide" class="fa fa-times remove"></a>
                <a data-username="${user.nick.toLowerCase()}" class="user">${user.nick} <span class="badge"></span></a>
            </li>
        `)
    }*/
