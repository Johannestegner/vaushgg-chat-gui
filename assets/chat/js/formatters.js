...
import {GENERIFY_OPTIONS, FLAG_WHITELIST, GENERIFY_BLACKLIST, GENERIFY_BLACKLISTS} from './const'
...
class EmoteFormatter {
    ...
    format(chat, str, message=null){
        if (!gemoteregex || !subemoteregex) {
            const emoticons = Array.from(chat.emoticons).join('|');
            const subemotes = Array.from(chat.subemotes).join('|');
            const hiddenemotes = Array.from(chat.hiddenemotes).join('|');
            
            
            const suffixes = Object.keys(GENERIFY_OPTIONS).join('|');
            gemoteregex = new RegExp(`(?:^|\\s)(${emoticons}|${hiddenemotes})((?::(?:${suffixes}))*)(?=$|\\s)`, 'gm');
            subemoteregex = new RegExp(`(?:^|\\s)(${emoticons}|${hiddenemotes}|${subemotes})((?::(?:${suffixes}))*)(?=$|\\s)`, 'gm');        
        }
        ...
        if (!this.emotewidths) {
            this.emotewidths = {};
            this.emoteheights = {};
            const gemoteArray = [...chat.emoticons, ...chat.hiddenemotes]
            const subemoteArray = [...chat.emoticons, ...chat.subemotes, ...chat.hiddenemotes]
            //const emoteArray = [...chat.emoticons, ...chat.subemotes]

            const emoteArray = (message && message.user && message.user.hasAnyFeatures(UserFeatures.SUBSCRIBER, UserFeatures.ADMIN, UserFeatures.MODERATOR)) || (!message || !message.user) ? subemoteArray : gemoteArray;
        ...
        let userregex = (message && message.user && message.user.hasAnyFeatures(UserFeatures.SUBSCRIBER, UserFeatures.ADMIN, UserFeatures.MODERATOR)) || (!message || !message.user) ? subemoteregex : gemoteregex;
        // const emoteCount = ((str || '').match(userregex) || []).length
        // re-seed the rng for halloween effects each emote
        var i = 0;
        return str.replace(userregex, function(m) {
            ...
            let flag = "";
            if (modifierEffect !== "") {
                flag = flag_check(emote);
            }
            
            const suffixes = m.slice(index_of_suffix + 1).split(':').map(suffix => suffix.replace(/\s/g, ''));
            // includes any part of string? 
            // if its in the table then if its its in the table with the suffix
            // && BLACKLIST[input[0]].includes(suffixes)
            if (GENERIFY_BLACKLIST.includes(emote)) {
                for(i = 0; i < suffixes.length; i++) {
                    if (GENERIFY_BLACKLISTS[emote].includes(suffixes[i])) {
                        return ' <span title=' + emote + ' class="chat-emote chat-emote-' + emote + '">' + emote + '</span>';
                    }
                }
            }
            
            return ` <span class="generify-container ${flag} ${suffixes.map(suffix => GENERIFY_OPTIONS[suffix]).join(' ')}"><span title="${m}" class="chat-emote chat-emote-${emote}">${m} </span></span>`;
            }
        });
    }
...
function flag_check(emote) {
    
    if (!FLAG_WHITELIST.includes(emote)) {
        return "";
    } else if (FLAG_WHITELIST.includes(emote)) { 
        //return " flagg";
        return " flagredux";
...
class GreenTextFormatter {        
        format(chat, str, message=null){
        if(message.user && str.includes('&gt;')){
            var strp = str.split('&gt;');
            var strp2 = strp.slice(1);
            str = `${strp[0]}<span class="greentext">>${strp2.join(`>`)}</span>`;
        }
        return str;
    }
    ...
    format(chat, str, message=null){
        ...
        return str.replace(self.linkregex, function(url, scheme) {            
            ...
            if (m) {               
                ...
                const href = url.replace(/(https:\/\/)?(http:\/\/)?(www\.)?/gmi, '');           
                url = url.replace(/(https:\/\/)?(http:\/\/)?/gmi, '');
                return `<a target="_blank" class="externallink ${extraclass}" href=https://${url} rel="nofollow noreferrer noopener">${href}</a>${extra}`;
            }
            return url;
        });
    }
